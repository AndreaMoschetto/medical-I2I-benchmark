...
from src.utils import CHECKPOINTS_PATH, DATAPATH, OUTPUT_DIR, compute_ssim_from_dataset, ensure_checkpoints_dir, generate_and_save_predictions, normalize_image
        ...
        for e in trange(n_epochs, desc="Epochs"):
            ...
            if e % 5 == 0 or e == n_epochs - 1 or batch_val_loss < best_val_loss:
                ...
                if batch_val_loss < best_val_loss:
                    ...
                    run.log({
                        "best_model_generations": [run.Image(grid, caption=f"Epoch {e+1} - Best")]
                    })
                else:
                    ...
                    run.log({
                        "each5e_generation": [run.Image(grid, caption=f"Epoch {e + 1}")]
                    })
